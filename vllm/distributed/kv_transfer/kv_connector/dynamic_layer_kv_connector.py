...
# class DynamicLayerKVConnector(KVConnectorBase):
...
class DynamicLayerKVConnector():
    ...
    def __init__(
        ...
        from vllm.distributed.kv_transfer.kv_pipe.dynamic_pynccl_pipe import (
            DynamicPyNcclPipe)
        logger.info(
            "Initializing PyNcclConfig under kv_transfer_config %s",
            self.config)
        ...
        # 根据当前的rank，和其它的所有rank建立一个pynccl pipe
        self.rank_to_producer_pipe: dict[int, DynamicPyNcclPipe] = {}
        self.rank_to_consumer_pipe: dict[int, DynamicPyNcclPipe] = {}
        for rank in range(torch.distributed.get_world_size()):
            if rank == local_rank:
                continue
            self.rank_to_producer_pipe[rank] = DynamicPyNcclPipe(
                local_rank=local_rank,
                config=self.config,
                port_offset=rank,
            )
            self.rank_to_consumer_pipe[rank] = DynamicPyNcclPipe(
                local_rank=local_rank,
                config=self.config,
                port_offset=rank,
            )

    def _send_kv_cache_to_rank(self, rank: int, kv_cache: torch.Tensor) -> None:
        self.rank_to_producer_pipe[rank].send_tensor(source_rank=self.rank, target_rank=rank, tensor=kv_cache)

    def _recv_kv_cache_from_rank(self, rank: int, callback: Callable[[torch.Tensor], None]) -> None:
        self.rank_to_consumer_pipe[rank].recv_tensor_with_callback(callback)

    def listen_kv_cache_from_rank(self, rank: int, callback: Callable[[torch.Tensor], None]) -> None:
