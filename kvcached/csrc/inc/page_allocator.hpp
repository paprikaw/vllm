...
using BroadcastMapCallback =
    std::function<void(int64_t, const std::vector<offset_t> &, int64_t,
                       int64_t)>;
...
public:
  ...
  void cap_blocks(int64_t max_num_blocks);
...
private:
  ...
  int64_t virtual_pages_per_layer_;
  int64_t page_size_;
  int64_t map_page_size_;
  int64_t world_size_;
  int64_t pp_rank_;
  int64_t num_kv_buffers_;
  int64_t layer_group_granularity_;
  int64_t num_layer_groups_;
  bool block_level_layer_groups_;
  int64_t group_id_;
  bool async_sched_;
