...
if [[ "$LOCAL_MODE" -eq 0 && "$(short_host)" != "$TARGET_HOST" ]]; then
  exec ssh "$TARGET_HOST" \
    "cd '$ROOT_DIR' && ROOT_DIR='$ROOT_DIR' VENV_ACTIVATE='$VENV_ACTIVATE' PYTHON_BIN='$PYTHON_BIN' TARGET_HOST='$TARGET_HOST' CONFIG_PATH='$CONFIG_PATH' RUN_LOG_DIR='$RUN_LOG_DIR' TMP_ROOT='$TMP_ROOT' PORT='$PORT' RUN_TIMEOUT_SECONDS='$RUN_TIMEOUT_SECONDS' CLEANUP_ON_EXIT='$CLEANUP_ON_EXIT' VLLM_EXP_SERVER_READY_TIMEOUT='${VLLM_EXP_SERVER_READY_TIMEOUT:-}' bash '$ROOT_DIR/scripts/run_pp4_shrink_rdt_race.sh' --local"
fi
