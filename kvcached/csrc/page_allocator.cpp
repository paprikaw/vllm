...
void InternalPage::cap_blocks(int64_t max_num_blocks) {
  if (end_block <= max_num_blocks) {
    return;
  }
  end_block = std::max(start_block, max_num_blocks);
  free_list.erase(
      std::remove_if(free_list.begin(), free_list.end(),
                     [max_num_blocks](int64_t block_id) {
                       return block_id >= max_num_blocks;
                     }),
      free_list.end());
  num_kv_blocks = static_cast<int64_t>(free_list.size());
}
...
PageAllocator::PageAllocator(int64_t num_layers, int64_t mem_size_per_layer,
    ...
    : num_layers_(num_layers), mem_size_per_layer_(mem_size_per_layer),
      ...
      layer_group_granularity_(layer_group_granularity <= 0
                                   ? 1
                                   : std::min(layer_group_granularity,
                                              num_layers)),
...
std::shared_ptr<InternalPage> PageAllocator::alloc_page() {
  ...
  while (page_id == -1) {
    ...
    // Wait for the background preallocation worker to publish reserved pages.
    cond_.wait(lock);
  }
...
bool PageAllocator::resize(int64_t new_mem_size) {
  int64_t new_num_pages = ceil_div(new_mem_size, page_size_);
...
void PageAllocator::map_pages(const std::vector<page_id_t> &page_ids) {
  ...
  if ((world_size_ > 1 || should_use_worker_ipc()) && broadcast_map_callback_) {
    ...
    broadcast_map_callback_(world_size_, offsets, pp_rank_, group_id_);
  } else {
...
void PageAllocator::unmap_pages(const std::vector<page_id_t> &page_ids) {
  ...
  if ((world_size_ > 1 || should_use_worker_ipc()) &&
    ...
    // callback
    broadcast_unmap_callback_(world_size_, offsets, pp_rank_, group_id_);
  } else {
