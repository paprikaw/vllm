...
def _set_cuda_device(device_index: int | None) -> None:
...
def _get_num_pp_stages() -> int:
    raw = os.getenv("KVCACHED_NUM_PP_STAGES", "1")
    try:
        value = int(raw)
    except ValueError:
        value = 1
...
def _iter_worker_targets(tp_size: int,
                         pp_rank: int = 0) -> Iterable[tuple[int, int]]:
    if pp_rank < 0:
        for pp in range(_get_num_pp_stages()):
...
async def _broadcast_map_to_kv_tensors(tp_size: int,
    ...
    map_message = {"cmd": "map_to_kv_tensors", "offsets": offsets,
                   "group_id": group_id}
    targets = list(_iter_worker_targets(tp_size, pp_rank))
    tasks = [
        _send_and_receive_message(rank, map_message, target_pp_rank)
        for rank, target_pp_rank in targets
    ]
...
async def _broadcast_unmap_from_kv_tensors(tp_size: int,
    ...
    for (rank, target_pp_rank), response in zip(targets, responses):
        if isinstance(response, Exception):
            raise RuntimeError(
                f"Worker pp{target_pp_rank}/tp{rank} failed to unmap: "
                f"{response}")
        elif not isinstance(response,
                            dict) or response.get("status") != "success":
            raise RuntimeError(
    ...
    for (rank, target_pp_rank), response in zip(targets, responses):
        if isinstance(response, Exception):
            all_created = False
                            ...
                            dict) or response.get("status") != "success":
            raise RuntimeError(
                f"Worker pp{target_pp_rank}/tp{rank} failed to check KV "
                f"tensors created: {response}"
            )
