...
CONFIG_PATH="${CONFIG_PATH:-$ROOT_DIR/vllm_exp/configs/qwen_pp4_autoscaling_expand_1to2_smoke.yaml}"
RUN_LOG_DIR="${RUN_LOG_DIR:-$ROOT_DIR/new-logs/qwen32b-pp4-expand-1to2-smoke-a100-$(date +%Y%m%d-%H%M%S)}"
TMP_ROOT="${TMP_ROOT:-/home/bxb1/data/tmp/vllm_exp/qwen_expand_1to2_smoke}"
PORT="${PORT:-8000}"
...
usage() {
  cat <<EOF
Usage:
  bash scripts/run_qwen_expand_1to2_smoke.sh [--local]
