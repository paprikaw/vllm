...
For vLLM, the native KVCacheD integration is selected by default when
`ENABLE_KVCACHED=true`. To compare against vLLM's existing VM/dynamic KV cache
implementation without uninstalling kvcached, switch the vLLM integration mode:

```bash
# KVCacheD native vLLM patches: ElasticBlockPool + KV tensor allocation.
export KVCACHED_VLLM_INTEGRATION_MODE=native

# Baseline: keep vLLM's existing VM/dynamic KV cache path untouched.
export KVCACHED_VLLM_INTEGRATION_MODE=vm
```
