...
from kvcached.integration.vllm.modes import (
    describe_vllm_integration_mode,
    enable_vllm_kvcached_native,
)
...
def _patch_vllm(_vllm: types.ModuleType) -> None:
    ...
    if not enable_vllm_kvcached_native():
        logger.info(
            "Skipping kvcached vLLM native patches "
            "(mode=%s, ENABLE_KVCACHED may be false)",
            describe_vllm_integration_mode(),
        )
        return
